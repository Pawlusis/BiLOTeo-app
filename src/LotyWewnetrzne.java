...
public class LotyWewnetrzne extends JFrame
{
	...
	public LotyWewnetrzne(Logowanie type1)
	{
		Container c =getContentPane();
		c.setLayout(new BorderLayout());
		String[] sItem1={"Wroclaw"};
		String[] sItem2 ={"Gdynia","Warszawa-Modlin","Warszawa-Okecie","Bydgoszcz","Gdansk","Poznan","Krakow","Katowice","Lublin","Rzeszow","Lodz","Olsztyn","Radom"};
		...
		LBookingDetails = new JLabel("<html><b><font color=\"#C71585\">Zamowienie</font></b></html>");
		LFrom = new JLabel("Z  :");
		LTo = new JLabel("Do :");
		LBookingDate = new JLabel("Data odlotu:");
		LClass = new JLabel("Klasa :");
		...
		LPassengerDetails=new JLabel("Wybór Pasażerów");

		LAdult = new JLabel("Dorośli(12+)");

		LChildren = new JLabel("Dzieci(2-11)");
		LInfant = new JLabel("Niemowlęta(<2)");
		...
		LNotes = new JLabel("<html><body><p> UWAGA: Rezerwacje dokonywane przy użyciu międzynarodowych kart kredytowych <p> zostały tymczasowo zawieszone. Ta usługa <p> zostanie wkrótce wznowiona, a na naszej stronie zostanie umieszczone powiadomienie <p>. niedogodności dla naszych pasażerów. </body> </html>");
		...
		LNotes.setBounds(-72,244,380,180);
	...
	{
		Logowanie type1=null;
		new LotyWewnetrzne(type1);
	}
}
...
class button3 implements ActionListener
{
	LotyWewnetrzne type;
	Logowanie type1;
	button3(LotyWewnetrzne type, Logowanie type1)
	{
		this.type = type;
		this.type1 = type1;
	}
	public void actionPerformed(ActionEvent e)
	{
		...
		if(sClass.equals("Economic"))
		{
			...
			{
				JOptionPane.showMessageDialog(null, "Odmowa Dostępu");
				System.exit(0);
			}
		}
		else
...
//read from data
		...
		if(iSeatCount > 60)
		{
			JOptionPane.showMessageDialog(null,"Brak wolnych miejsc.Przepraszamy!");
		}
		else
		{
			int iChoice = JOptionPane.showConfirmDialog(null,"Wolne miejsca. chcesz zarezerwować?");
			if(iChoice == JOptionPane.YES_OPTION)
			{
				new DrukowanieBiletu(sFrom, sTo, sClass, iAdult, iChildren, iInfant, sBookingDate, iPrice, sTime);
			try
			{
//write into data
